...
static u16 LCD_Buffer[LCD_WIDTH * LCD_HEIGHT];



static void Graphic_setFGcolor(u16 color)
{
	FG_color = color;
}

static void Graphic_setBGcolor(u16 color)
{
	BG_color = color;
}


static int ssd1306_DrawPixel(u16 x, u16 y, u16 color) {

	if ( x > lcd.width || y > lcd.height ) {

		printf("%s: out of borders - x = %d; y = %d\n", __FUNCTION__, x, y);
		return -1;
	}

	// Set color
	LCD_Buffer[x + (y * lcd.width)] = color;
...
{
	ssd1306_DrawPixel(X, Y, FG_color);
}
...
void Graphic_drawCircle(_Point center, u16 r){
	...
	// calculate 8 special point(0, 45, 90, 135, 180, 225, 270 degree) display them 
...
static void Graphic_ClearScreen(void){
	...
	//printf("%s\n", __FUNCTION__);
	memset(LCD_Buffer, BG_color, sizeof(LCD_Buffer));
}
...
static void Graphic_UpdateScreen(void){
	...
	u8 *p_dst = lcd.fbp;
	u16 *p_src = LCD_Buffer;


	memcpy(p_dst, (u8 *)p_src, lcd.width * lcd.height * 2);
/*
	for (i=0; i < lcd.height; i++) {
		memcpy(p_dst, (u8 *)p_src, lcd.width *2);
		p_dst += lcd.width *2;
		p_src += lcd.width;
	}
*/	
...
{	
	...
	for(angle = 0.0; ; ){
		...
		Graphic_ClearScreen();
		Graphic_setFGcolor(clWhite);
		Graphic_setBGcolor(clBlack);
		Graphic_drawCircle(center, radius);
		Graphic_drawCircle(center, radius + 2);

		for(angle2 = 0.0; angle2 <= 360; ){
			Graphic_setFGcolor(clYellow);
			Graphic_drawLine_(
